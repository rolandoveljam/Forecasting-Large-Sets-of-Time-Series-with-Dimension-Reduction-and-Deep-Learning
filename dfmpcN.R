...
#'
#' The function estimates the Dynamic Factor Model by Principal Components and by the estimator of Lam et al. (2011). and provides estimates of the models using arimaSpec()
#'
...
#'
#' @export
dfmpcN <- function(x, stand = 0, mth = 4, r, lagk = 0){
  ...
  for (i in 1:ncol(F)){
    f <- F[,i]
    outputF <- arimaSpec(f)
    armamdlF[i,c(1,2,6)] <- outputF$order[c(1,3,2)]
    armamdlF[i,5]=1
  }
  ...
  for (i in 1:ncol(E)){
    e <- E[,i]
    outputE <- arimaSpec(e)
    armamdlE[i,c(1,2,6)] <- outputE$order[c(1,3,2)]
    armamdlE[i,5]=1
  }
